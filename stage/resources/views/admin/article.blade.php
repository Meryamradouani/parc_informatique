...
<body>
    <div class="container mt-5">
        <div class="table-title">
            <div class="row">
                <div class="col-sm-8"><h2>Liste des <b>Articles</b></h2></div>
                <div class="col-sm-6">
                    <a href="#addArticleModal" class="btn btn-success" data-toggle="modal">
                        <i class="fa fa-plus" aria-hidden="true"></i> Créer un Nouvel Article
                    </a>
                </div>
            </div>
        </div>
        ...
        <table id="articleTable" class="table table-striped table-hover table-bordered">
            <thead>
                <tr>
                ...
                @foreach($articles as $article)
                    <tr>
                        ...
                        <td>
                            ...
                            <button class="btn btn-info detailsBtn" data-toggle="modal" data-target="#detailsModal"
                                    data-numeroserie="{{ $article->numeroserie }}"
                                    data-marque="{{ $article->marque }}"
                                    data-model="{{ $article->model }}"
                                    data-libelle="{{ $article->libelle }}"
                            ...
                            <button type="button" class="btn btn-danger deleteBtn" data-id="{{ $article->code_article }}">
                                <i class="fas fa-trash"></i>
                            </button>
                            <form id="deleteForm{{ $article->code_article }}" action="{{ route('admin.articles.destroy', $article->code_article) }}" method="POST" style="display: none;">
                                @csrf
                                @method('DELETE')
                            </form>
                        </td>
    ...
    <div class="modal fade" id="addArticleModal" tabindex="-1" role="dialog" aria-labelledby="addArticleModalLabel" aria-hidden="true">
        <div class="modal-dialog" role="document">
            <div class="modal-content">
                ...
                <form id="addArticleForm" method="POST" action="{{ route('articles.store') }}">
                    @csrf
                    @if ($errors->any())
    ...
    <script>
        $(document).ready(function() {
            $('#articleTable').DataTable({
                "language": {
                    "url": "//cdn.datatables.net/plug-ins/9dcbecd42ad/i18n/French.json" // Optionnel: fichier de langue française pour DataTables
                }
            });
        ...
        $('#editModal').on('show.bs.modal', function (event) {
            ...
            modal.find('#editForm').attr('action', '/admin/articles/' + id); // Assurez-vous que l'URL est correcte
        });
